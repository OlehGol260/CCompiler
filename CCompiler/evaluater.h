...
private:
	...
	std::shared_ptr<Variable> InnerEvaluate(std::shared_ptr<LexemeInterface> root) const;

	template<typename T>
	T EvaluateExpression(std::shared_ptr<LexemeInterface> root) const
	{
		return 0;
		//return static_cast<T>(InnerEvaluate(root));
	}

	template<class VarT, class ResultT>
	void SetAndEvaluate(std::shared_ptr<Variable> var, std::shared_ptr<Lexeme> lexem) const
	{
		auto int_var = std::static_pointer_cast<VarT>(var);
		int_var->set_value(EvaluateExpression<ResultT>(lexem->right()));
